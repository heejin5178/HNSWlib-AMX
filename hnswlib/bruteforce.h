 ...
 public:
    ...
    searchKnn(const void *query_data, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const {
        ...
        if (amxdistfunc_) {
            ...
            const void* ptrs[BATCH];
            float res[BATCH] __attribute__((aligned(64)));
            size_t dim = *(size_t*)dist_func_param_;

            for (size_t batch_start = 0; batch_start < cur_element_count; batch_start += BATCH) {
                size_t count = std::min(BATCH, cur_element_count - batch_start);

                for (size_t j = 0; j < count; j++) {
                    ptrs[j] = data_ + size_per_element_ * (batch_start + j);
                }

                memset(res, 0, sizeof(float) * count);
                amxdistfunc_(ptrs, query_data, &dim, count, 1, res);

                for (size_t j = 0; j < count; j++) {
                    dist_t dist = res[j];
                    labeltype label = *((labeltype*)(data_ + size_per_element_ * (batch_start + j) + data_size_));
