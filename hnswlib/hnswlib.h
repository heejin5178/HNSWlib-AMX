...
// Global log stream: defaults to std::cout, can be redirected via set_log_stream()
inline std::ostream*& _log_stream_ptr() {
    static std::ostream* stream = &std::cout;
    return stream;
}
inline void set_log_stream(std::ostream* stream) {
    _log_stream_ptr() = stream;
}
inline std::ostream& log_stream() {
    return *_log_stream_ptr();
}
