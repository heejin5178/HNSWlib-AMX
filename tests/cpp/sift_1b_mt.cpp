...
test_vs_recall(
    ...
    log << "Bruteforce (Bf16InnerProductSpace, MT)\t" << recall << "\t" << time_us_per_query << " us/query\t" << total_time_ms << " ms total\n";
#else
    log << "Bruteforce (InnerProductSpace float, MT)\t" << recall << "\t" << time_us_per_query << " us/query\t" << total_time_ms << " ms total\n";
#endif
}
...
#if defined(USE_AMX_BF16)
    log << "Bruteforce (Bf16InnerProductSpace, MT)\tQPS: " << qps << "\tTotal queries: " << total_queries << ", recall: " << recall  << "\n";
#else
    log << "Bruteforce (InnerProductSpace float, MT)\tQPS: " << qps << "\tTotal queries: " << total_queries << ", recall: " << recall << "\n";
#endif
}
